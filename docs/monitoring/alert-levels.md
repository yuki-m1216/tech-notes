...
Infoレベルのメトリクスはダッシュボードで定期確認します。ダッシュボード設計の詳細は[ダッシュボード設計のベストプラクティス](dashboard-design.md)を参照してください。
...
アラートレベル設定でよくある失敗パターンについては、[よくある失敗例](common-mistakes.md)を参照してください。特に以下が重要です:

- 全てをCriticalにする（アラート疲れの原因）
- InfoレベルをSNS通知にする（通知の埋もれ）
- WarningやInfoに過度に迅速な対応を求める（開発生産性の低下）
- 閾値が低すぎる・評価期間が短すぎる（誤検知の多発）
...
- [Monitoring Distributed Systems（Google SRE Book）](https://sre.google/sre-book/monitoring-distributed-systems/) - アラート疲れの問題と適切なアラートレベル設計について
- [Alerting Principles（PagerDuty）](https://response.pagerduty.com/oncall/alerting_principles/) - オンコール対応とアラート優先度の考え方
