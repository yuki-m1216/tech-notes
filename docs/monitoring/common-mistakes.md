...
## 11. アクションを起こせないアラートを作る

### 問題

受け取っても具体的な対応ができないアラートを設定することで、アラート疲れや無視につながります。

**失敗例:**

```
❌ AWS側の障害でエンジニアにできることがない → Critical通知
❌ 情報共有のみで対応不要 → Slack通知
❌ 対応手順が誰にも分からない → Critical通知
```

### 対策

アラート設計時に「このアラートを受けたら何をするか」を必ず確認します。

```
✅ アクションが明確 → 適切なレベルで通知
✅ 情報共有のみ → Infoレベル（ダッシュボード確認）
✅ 対応手順が不明 → まずランブックを整備、または通知を見直し
```

## 12. インシデント後にアラートを見直さない

### 問題

インシデントが発生しても振り返りを行わず、同じ問題を繰り返します。

**失敗例:**

```
❌ インシデント対応で疲弊し、振り返りをスキップ
❌ 「今回は特殊なケースだった」と改善を見送る
❌ 振り返りはしたが、アラート改善のアクションが実行されない
```

**症状:**

- 同じ原因でインシデントが再発する
- アラートが鳴ったのに対応が遅れた経験が繰り返される
- 「このアラート、前も役に立たなかった」という声が出る

### 対策

インシデント後は必ずポストモーテムを実施し、アラートの有効性を評価します。

```
✅ Criticalアラート発生後は必ずポストモーテムを実施
✅ アラートが適切だったか（タイミング、通知先、情報量）を評価
✅ 改善アクションを記録し、期限を決めて実行
✅ 次回のインシデントで改善が活かされたか確認
```
