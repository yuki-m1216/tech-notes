...
問題は上位レイヤーから下位レイヤーへ伝播します。多層的に監視することで、迅速に根本原因を特定できます。

```mermaid
graph TD
    A[ユーザー体験<br/>レスポンス遅延] --> B[アプリケーション<br/>データベースクエリ遅延]
    B --> C[インフラ<br/>RDS CPU 100%]
...
1. **ユーザー体験**: Syntheticsで決済ページのレスポンスタイムが10秒以上
2. **アプリケーション**: API Gatewayで決済APIのレスポンスタイムが9秒
3. **インフラ**: RDS のCPU使用率が100%に張り付き

→ **根本原因**: RDS のスロークエリ

### 例: 可用性の問題調査

1. **ユーザー体験**: Syntheticsでサービスにアクセス成功（200 OK）
2. **アプリケーション**: API Gatewayのエラー率が上昇（一部リクエストが失敗）
3. **インフラ**: ALBのTargetHealthで2台中1台がUnhealthy

→ **根本原因**: EC2インスタンス1台の障害（Syntheticsは成功したが、一部ユーザーは影響を受けている）
