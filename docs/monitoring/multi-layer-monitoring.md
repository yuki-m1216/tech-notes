...
## ゴールデンシグナル（The Four Golden Signals）

各レイヤーで「何を監視するか」を決める際の指針として、Google SRE Bookで提唱された**ゴールデンシグナル**があります。

### 4つのシグナル

| シグナル | 説明 | 監視例 |
|---------|------|-------|
| **レイテンシ（Latency）** | リクエストの処理にかかる時間 | API レスポンスタイム、ページロード時間 |
| **トラフィック（Traffic）** | システムへのリクエスト量 | リクエスト数/秒、同時接続数 |
| **エラー（Errors）** | 失敗したリクエストの割合 | 5xxエラー率、例外発生数 |
| **サチュレーション（Saturation）** | リソースの使用率・逼迫度 | CPU使用率、メモリ使用率、キュー長 |

### なぜゴールデンシグナルが重要か

- **エンドユーザー視点**: レイテンシとエラーは直接ユーザー体験に影響する
- **キャパシティ把握**: トラフィックとサチュレーションでシステムの限界を理解できる
- **問題の早期発見**: 4つを組み合わせることで、問題の予兆から影響まで把握できる

### 各レイヤーでのゴールデンシグナル

| レイヤー | レイテンシ | トラフィック | エラー | サチュレーション |
|---------|-----------|-------------|--------|-----------------|
| ユーザー体験 | ページロード時間 | アクティブユーザー数 | JSエラー率 | - |
| アプリケーション | API レスポンスタイム | リクエスト数/秒 | 5xxエラー率 | Lambda同時実行数 |
| インフラ | DBクエリ時間 | 接続数 | 接続エラー | CPU/メモリ使用率 |

### SLI/SLOとの関係

ゴールデンシグナルは**SLI（Service Level Indicator）** の基礎となります。

| 用語 | 説明 | 例 |
|-----|------|-----|
| **SLI** | サービス品質を測定する具体的な指標 | 「APIレスポンスタイムの99パーセンタイル」 |
| **SLO** | SLIの目標値（ユーザーとの約束） | 「99パーセンタイルが200ms以下」 |
| **SLA** | SLOを契約として明文化したもの | 「SLO未達時は返金」 |
...
ゴールデンシグナル: レイテンシ
        ↓
SLI: APIレスポンスタイムの99パーセンタイル
        ↓
SLO: 99パーセンタイルが200ms以下を99.9%の時間で維持
        ↓
アラート: SLO違反の予兆でWarning、違反でCritical
```

SLI/SLOを定義することで、「何をどのレベルで監視すべきか」が明確になり、アラートの閾値設定に客観的な根拠を持たせることができます。

!!! note "詳細について"
    SLI/SLOの詳細な設計方法（エラーバジェット、バーンレートなど）については、[Google SRE Workbook - Alerting on SLOs](https://sre.google/workbook/alerting-on-slos/) を参照してください。

### アラート設計への活用

ゴールデンシグナルを基準にアラートを設計することで、重要なメトリクスを漏れなく監視できます。

**優先順位の考え方:**

1. **エラー**: ユーザー影響が最も直接的 → Critical候補
2. **レイテンシ**: SLO違反に直結 → Critical候補
3. **サチュレーション**: 問題の予兆 → Warning候補
4. **トラフィック**: キャパシティプランニング → Info（ダッシュボード確認）
